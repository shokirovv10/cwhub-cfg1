...
from app.models import (
    User, Config, Category, Order, OrderStatus, Payment, PaymentStatus,
    Review, PaymentSettings, SiteSettings, Admin
)
from app.forms import (
    ConfigForm, CategoryForm, PaymentSettingsForm, SiteSettingsForm, RejectOrderForm
)
...
def dashboard():
    stats = {
        ...
        "pending_payments": Order.query.filter_by(status=OrderStatus.UNDER_REVIEW.value).count(),
        "rejected_payments": Order.query.filter_by(status=OrderStatus.REJECTED.value).count(),
    }
    paid_orders = Order.query.filter_by(status=OrderStatus.PAID.value).all()
    stats["total_revenue"] = sum(float(o.amount) for o in paid_orders)
...
def config_new():
    ...
    if form.validate_on_submit():
        if not form.cfg_file.data:
            flash("CFG fayl majburiy.", "danger")
            return render_template("admin/config_form.html", form=form, mode="new")
        try:
            cfg_filename = save_uploaded_file(
                form.cfg_file.data, "configs", {"cfg", "zip"},
            ...
            if form.screenshot.data:
                screenshot_filename = save_uploaded_file(
                    form.screenshot.data, "screenshots", {"jpg", "jpeg", "png", "webp"},
            ...
            slug = base_slug
            n = 1
            while Config.query.filter_by(slug=slug).first():
                n += 1
                slug = f"{base_slug}-{n}"
            ...
            config = Config(
                ...
                category_id=form.category_id.data, author=form.author.data,
                cs_version=form.cs_version.data, demo_video_url=form.demo_video_url.data,
                cfg_file_filename=cfg_filename, screenshot_filename=screenshot_filename,
                is_active=form.is_active.data, is_hidden=form.is_hidden.data,
            )
            db.session.add(config)
            db.session.commit()
            flash("Config qo'shildi.", "success")
    ...
    if form.validate_on_submit():
        try:
            ...
            if form.cfg_file.data:
                config.cfg_file_filename = save_uploaded_file(
                    form.cfg_file.data, "configs", {"cfg", "zip"},
                    {"text/plain", "application/zip", "application/x-zip-compressed", "application/octet-stream"},
                )
            if form.screenshot.data:
                config.screenshot_filename = save_uploaded_file(
                    form.screenshot.data, "screenshots", {"jpg", "jpeg", "png", "webp"},
                    {"image/jpeg", "image/png", "image/webp"},
                )

            db.session.commit()
            flash("Config yangilandi.", "success")
            return redirect(url_for("admin.config_list"))
        except ValueError as e:
            flash(str(e), "danger")

    return render_template("admin/config_form.html", form=form, mode="edit", config=config)
...
def config_delete(config_id):
    ...
    db.session.commit()
    flash("Config o'chirildi.", "info")
    return redirect(url_for("admin.config_list"))


@admin_bp.route("/configs/<int:config_id>/toggle-hide", methods=["POST"])
...
@admin_bp.route("/configs/<int:config_id>/toggle-active", methods=["POST"])
...
def categories():
    ...
    if form.validate_on_submit():
        ...
        if Category.query.filter_by(slug=slug).first():
            flash("Bu kategoriya allaqachon mavjud.", "danger")
        else:
...
def category_delete(cat_id):
    ...
    if cat.configs.count() > 0:
        flash("Bu kategoriyada configlar mavjud, avval ularni ko'chiring yoki o'chiring.", "danger")
    else:
        db.session.delete(cat)
        db.session.commit()
        flash("Kategoriya o'chirildi.", "info")
    return redirect(url_for("admin.categories"))
...
def user_toggle_block(user_id):
    user = User.query.get_or_404(user_id)
    if user.is_admin:
        flash("Adminlarni bloklab bo'lmaydi.", "danger")
...
def order_list():
    ...
    return render_template("admin/orders.html", orders=all_orders, status=status, statuses=[s.value for s in OrderStatus])


@admin_bp.route("/payments/<int:order_id>/receipt")
def view_receipt(order_id):
    """Admin uchun chekni ko'rish - faylga to'g'ridan-to'g'ri public kirish yo'q, faqat shu route orqali."""
    order = Order.query.get_or_404(order_id)
    if not order.payment or not order.payment.receipt:
        abort(404)
    try:
        file_path = safe_upload_path("receipts", order.payment.receipt.filename)
    except ValueError:
        abort(403)
...
def payments():
    """To'lovlarni tekshirish bo'limi - UNDER_REVIEW statusidagi orderlar."""
    pending = Order.query.filter_by(status=OrderStatus.UNDER_REVIEW.value).order_by(Order.created_at.asc()).all()
    reject_form = RejectOrderForm()
    return render_template("admin/payments.html", orders=pending, reject_form=reject_form)
...
def payment_approve(order_id):
    ...
    if not order.payment:
        flash("Bu order uchun to'lov topilmadi.", "danger")
        return redirect(url_for("admin.payments"))
    payment_service.approve_payment(order.payment, current_user.admin_profile.id)
    flash(f"Order #{order.id} tasdiqlandi. Foydalanuvchi endi yuklab olishi mumkin.", "success")
    return redirect(url_for("admin.payments"))
...
def payment_reject(order_id):
    ...
    payment_service.reject_payment(order.payment, current_user.admin_profile.id, reason=reason)
    flash(f"Order #{order.id} rad etildi.", "info")
    return redirect(url_for("admin.payments"))
...
def payment_settings():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("To'lov sozlamalari yangilandi.", "success")
...
def site_settings():
    ...
    if form.validate_on_submit():
        form.populate_obj(settings)
        db.session.commit()
        flash("Sayt sozlamalari yangilandi.", "success")
